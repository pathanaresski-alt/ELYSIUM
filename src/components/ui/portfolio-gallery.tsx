...
export function PortfolioGallery({
  ...
  return (
    <section
      ...
      id="archives"
    >
      <div className="max-w-7xl mx-auto rounded-sm border border-white/5 overflow-hidden">
        {/* Header Section */}
        <div className="relative z-10 text-center pt-12 pb-6 px-8">
          <h2 className="text-4xl md:text-6xl font-bento-title text-foreground mb-6 text-balance">{title}</h2>

          <Link
            to={archiveButton.href}
            className="inline-flex items-center gap-3 bg-brand-orange text-white px-8 py-3 rounded-sm font-black uppercase text-[10px] tracking-widest transition-all transform hover:scale-110 active:scale-95 group mb-12"
          >
        ...
        <div className="hidden md:block relative overflow-hidden h-[300px] -mb-[50px]">
          <div className={`flex ${spacing} pb-4 pt-20 items-end justify-center`}>
            {images.map((image, index) => {
              // Calculate stagger height - peak in middle, descending to edges
              const totalImages = images.length
              const middle = Math.floor(totalImages / 2)
              const distanceFromMiddle = Math.abs(index - middle)
              const staggerOffset = maxHeight - distanceFromMiddle * 15
              ...
              const yOffset = isHovered ? -80 : isOtherHovered ? 0 : -staggerOffset
              ...
              return (
                <motion.div
                  ...
                  initial={{
                    transform: `perspective(5000px) rotateY(-45deg) translateY(100px)`,
                    opacity: 0,
                  }}
                  animate={{
                ...
                >
                  <div
                    className="relative aspect-video w-56 md:w-64 lg:w-80 rounded-sm overflow-hidden transition-transform duration-300 group-hover:scale-105 border border-white/10"
                    style={{
                      boxShadow: `
                        rgba(0, 0, 0, 0.1) 0.796192px 0px 0.796192px 0px,
                        rgba(0, 0, 0, 0.2) 2.41451px 0px 2.41451px 0px,
                        rgba(0, 0, 0, 0.3) 6.38265px 0px 6.38265px 0px,
                        rgba(255, 77, 0, 0.2) 20px 0px 20px 0px
                      `,
                    }}
                  >
                    <img
              ...
              .map((_, i) => (
                ...
                >
                  {images.map((image, index) => (
                    ...
                    >
                      <div
                        className="relative aspect-video w-64 rounded-sm overflow-hidden transition-transform duration-300 group-hover:scale-105 border border-white/10 shadow-xl"
                      >
                        <img
                          src={image.src || "/placeholder.svg"}
                          alt={image.alt}
