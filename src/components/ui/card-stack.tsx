      ...
      {showDots ? (
        <div className="mt-6 flex items-center justify-center gap-3">
          ...
          {activeItem.href && (
            <Link
              to={activeItem.href}
              className="text-white/40 hover:text-brand-orange transition"
  ...
  return (
    <div className="relative h-full w-full bg-black">
      ...
      <div className="absolute inset-0">
        {item.imageSrc ? (
          <img
            src={item.imageSrc}
            alt={item.title}
            className="h-full w-full object-cover grayscale brightness-75 transition-all duration-700 hover:grayscale-0 hover:brightness-100"
            draggable={false}
            loading="eager"
          />
        ) : (
      ...
      <div className="relative z-10 flex h-full flex-col justify-end p-8">
        <div className="truncate text-2xl font-black uppercase italic tracking-tighter text-white">
          {item.title}
        </div>
        {item.description ? (
          <div className="mt-2 line-clamp-2 text-[10px] font-bold uppercase tracking-widest text-white/60">
            {item.description}
          </div>
        ) : null}
      </div>
